...
for i = 1:numofFiles
    ...
    for j = 1:size(stats_x_position,2)
        aa = lm97(lm97(:,11)>(stats_x_position(j)-step_size*2)/100,:);
        bb = aa(aa(:,11)<(stats_x_position(j)+step_size*2)/100,:);
        % orientation
    ...
    % figure(f7);hold on;
    % obj_array(i,4,1) = scatter(lm97(:,18),abs(lm97(:,13)),'markeredgecolor',[r(i)/2 g(i)/2 b(i)/2],'markerfacecolor',[r(i) g(i) b(i)]);
